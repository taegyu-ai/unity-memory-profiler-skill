...
1. **Phase A — Overview (once)**: `Initialize` → `GetMemoryOverview` + `GetTopUnityObjectCategories` + `GetNativeSubsystems` (batch). Build one **ranked candidate list** sorted by resident %. **State the analyzed snapshot at the top of the result** (`Analyzed snapshot: <snapshotName>` from `Initialize`).
...
3. Present the **ranked diff**: **state the compared pair at the top** (`Compared: A=<snapshotNameA> vs B=<snapshotNameB>` from `InitializeComparison`); lead with `new` and `grew`; the same **resident-first / GPU / Untracked** rules and Artifact/EditorOnly exclusions as Phase A carry over.
...
- **Scope-matching**: apply only Learned Entries whose `scope` matches the current snapshot. Match `platform`/`type`/`captureOrigin` as usual, and match `project` against `Initialize`'s `productName` (the captured project's name): `project=*` (the default) matches any project; a concrete `project` value applies only when it equals `productName`; if `productName` is null (legacy capture with no product name) drop the `project` dimension. Mismatched entries are skipped or marked low-confidence to prevent overfitting.
